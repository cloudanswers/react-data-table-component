...
function isRowExpanded(row, expandedRows = [], keyField = 'id') {
    const outerField = prop(row, keyField);
    if (outerField) {
    ...
    switch (action.type) {
        ...
        case 'EXPAND_ALL_ROWS': {
            ...
            if (mergeExpansions) {
                ...
                return Object.assign(Object.assign({}, state), { allExpanded: allChecked, expandedCount: expansions.length, expandedRows: expansions, toggleOnExpandedRowsChange });
            }
            return Object.assign(Object.assign({}, state), { allExpanded: allChecked, expandedCount: allChecked ? rowCount : 0, expandedRows: allChecked ? rows : [], toggleOnExpandedRowsChange });
        }
        ...
        case 'EXPAND_SINGLE_ROW': {
            ...
            if (singleExpand) {
                if (isExpanded) {
                    return Object.assign(Object.assign({}, state), { expandedCount: 0, allExpanded: false, expandedRows: [], toggleOnExpandedRowsChange });
                }
                return Object.assign(Object.assign({}, state), { expandedCount: 1, allExpanded: false, expandedRows: [row], toggleOnExpandedRowsChange });
            }
            if (isExpanded) {
                return Object.assign(Object.assign({}, state), { expandedCount: state.expandedRows.length > 0 ? state.expandedRows.length - 1 : 0, allExpanded: false, expandedRows: removeItem(state.expandedRows, row, keyField), toggleOnExpandedRowsChange });
            }
            return Object.assign(Object.assign({}, state), { expandedCount: state.expandedRows.length + 1, allExpanded: state.expandedRows.length + 1 === rowCount, expandedRows: insertItem(state.expandedRows, row), toggleOnExpandedRowsChange });
        }
        ...
        case 'EXPAND_MULTIPLE_ROWS': {
            ...
            if (mergeExpansions) {
                ...
                return Object.assign(Object.assign({}, state), { expandedCount: expansions.length, allExpanded: false, expandedRows: expansions, toggleOnExpandedRowsChange });
            }
            return Object.assign(Object.assign({}, state), { expandedCount: expandedRows.length, allExpanded: expandedRows.length === totalRows, expandedRows,
        ...
        case 'CLEAR_EXPANDED_ROWS': {
            ...
            return Object.assign(Object.assign({}, state), { allExpanded: false, expandedCount: 0, expandedRows: [], expandedRowsFlag });
        }
        case 'SORT_CHANGE': {
...
const ButtonStyle$1 = styled__default["default"].button `
	display: inline-flex;
	align-items: center;
	user-select: none;
	white-space: nowrap;
...
`;
function ExpanderButton({ name, disabled = false, expanded = false, expandableIcon, id, row, onToggled, }) {
    const icon = expanded ? expandableIcon.expanded : expandableIcon.collapsed;
    const handleToggle = () => onToggled && onToggled(row);
    return (React__namespace.createElement(ButtonStyle$1, { name: name, "aria-disabled": disabled, onClick: handleToggle, "data-testid": `expander-button-${id}`, disabled: disabled, "aria-label": expanded ? 'Collapse Row' : 'Expand Row', role: "button", type: "button" }, icon));
}
...
`;
function Row({ columns = [], conditionalRowStyles = [], defaultExpanderDisabled = false, dense = false, expandableIcon, expandableRows = false, expandableRowsComponent, expandableRowsComponentProps, expandableRowsHideExpander, expandOnRowClicked = false, expandOnRowDoubleClicked = false, highlightOnHover = false, id, expandableInheritConditionalStyles, keyField, onRowClicked = noop, onRowDoubleClicked = noop, onRowExpandToggled = noop, onSelectedRow = noop, onExpandedRow = noop, pointerOnHover = false, row, rowCount, rowIndex, selectableRowDisabled = null, expandableRowDisabled = null, selectableRows = false, selectableRowsComponent, selectableRowsComponentProps, selectableRowsHighlight = false, selectableRowsSingle = false, expandableRowsSingle = false, selected, expanded, striped = false, keepExpandableFirst = false, draggingColumnId, onDragStart, onDragOver, onDragEnd, onDragEnter, onDragLeave, }) {
    ...
    return (React__namespace.createElement(React__namespace.Fragment, null,
        React__namespace.createElement(TableRowStyle, { id: `row-${id}`, role: "row", striped: isStriped, highlightOnHover: highlightOnHover, pointerOnHover: !defaultExpanderDisabled && showPointer, dense: dense, onClick: handleRowClick, onDoubleClick: handleRowDoubleClick, className: classNames, selected: highlightSelected, style: style },
            keepExpandableFirst ? (React__namespace.createElement(React__namespace.Fragment, null,
                expandableCell,
                selectableCell)) : (React__namespace.createElement(React__namespace.Fragment, null,
                selectableCell,
                expandableCell)),
            columns.map(column => {
                if (column.omit) {
...
const ColumnText = styled__default["default"].div `
	...
	text-overflow: ellipsis;
`;
function TableCol({ column, disabled, draggingColumnId, selectedColumn = {}, sortDirection, sortIcon, sortServer, pagination, paginationServer, persistSelectedOnSort, selectableRowsVisibleOnly, onSort, onDragStart, onDragOver, onDragEnd, onDragEnter, onDragLeave, currentSortColumnId, currentSortDirection, }) {
...
const ColumnStyle$1 = styled__default["default"](CellBase) `
	flex: 0 0 48px;
	justify-content: center;
	align-items: center;
	user-select: none;
...
function ColumnCheckbox({ headCell = true, rowData, keyField, allSelected, mergeSelections, selectedRows, selectableRowsComponent, selectableRowsComponentProps, selectableRowDisabled, onSelectAllRows, }) {
    ...
    return (React__namespace.createElement(ColumnStyle$1, { className: "rdt_TableCol", headCell: headCell, noPadding: true },
...
const ButtonStyle = styled__default["default"].button `
	...
	${({ theme }) => theme.expanderButton.style};
`;
function ExpanderButtonHead({ name, disabled = false, expanded = false, expandableIcon, onToggled, }) {
    const icon = expanded ? expandableIcon.expanded : expandableIcon.collapsed;
    const handleToggle = onToggled;
    return (React__namespace.createElement(ButtonStyle, { name: name, "aria-disabled": disabled, onClick: handleToggle, disabled: disabled, "aria-label": expanded ? 'Collapse Row' : 'Expand Row', role: "button", type: "button" }, icon));
}
...
const ColumnStyle = styled__default["default"](CellBase) `
	...
	font-size: unset;
`;
function ColumnExpander({ headCell = true, rowData, keyField, allExpanded, mergeExpansions, expandedRows, expandableIcon, expandableRowDisabled, onExpandAllRows, }) {
    const indeterminate = expandedRows.length > 0 && !allExpanded;
    const rows = expandableRowDisabled ? rowData.filter((row) => !expandableRowDisabled(row)) : rowData;
    const isDisabled = rows.length === 0;
    const rowCount = Math.min(rowData.length, rows.length);
    ...
    return (React__namespace.createElement(ColumnStyle, { className: "rdt_TableCol", headCell: headCell, noPadding: true },
        React__namespace.createElement(ExpanderButtonHead, { name: "expand-all-rows", expanded: indeterminate || allExpanded, expandableIcon: expandableIcon, disabled: isDisabled, onToggled: handleExpandAll })));
}
...
function DataTable(props) {
    ...
    const index = React__namespace.useMemo(() => tableColumns.reduce((i, column) => {
        if (column.freeze) {
            i += 1;
        }
        return i;
    }, 0), [tableColumns]);
    const tableColumnsBasedOnTable = React__namespace.useMemo(() => {
        if (!isInnerTable) {
            return tableColumns;
        }
        return tableColumns.slice(0, index);
    }, [index, isInnerTable, tableColumns]);
    const freezedColumns = React__namespace.useMemo(() => {
        return tableColumns.slice(0, index);
    }, [index, tableColumns]);
    const [{ rowsPerPage, currentPage, selectedRows, expandedRows, allSelected, allExpanded, selectedCount, expandedCount, selectedColumn, sortDirection, toggleOnSelectedRowsChange, toggleOnExpandedRowsChange, }, dispatch,] = React__namespace.useReducer(tableReducer, {
        allSelected: false,
        allExpanded: false,
        selectedCount: 0,
        expandedCount: 0,
    ...
    const { persistSelectedOnSort = false, persistSelectedOnPageChange = false, persistExpandedOnSort = false, persistExpandedOnPageChange = false, } = paginationServerOptions;
    ...
    const handleExpandAllRows = React__namespace.useCallback((action) => {
        dispatch(action);
    }, []);
    const handleSelectedRow = React__namespace.useCallback((action) => {
        dispatch(action);
    }, []);
    const handleExpandedRow = React__namespace.useCallback((action) => {
        dispatch(action);
    }, []);
    ...
    const selectableHeaderCol = selectableRows &&
    ...
    const expandableHeaderCol = expandableRows &&
        (showExpandAll ? (React__namespace.createElement(CellBase, { style: { flex: '0 0 48px' } })) : (React__namespace.createElement(ColumnExpander, { allExpanded: allExpanded, expandedRows: expandedRows, rowData: visibleRows, keyField: keyField, mergeExpansions: mergeExpansions, onExpandAllRows: handleExpandAllRows, expandableIcon: expandableIcon, expandableRowDisabled: expandableRowDisabled })));
    return (React__namespace.createElement(styled.ThemeProvider, { theme: currentTheme },
        ...
        React__namespace.createElement("div", { style: { position: 'relative' } },
                    ...
                    React__namespace.createElement(TableStyle, { disabled: disabled, className: "rdt_Table", role: "table" },
                        showTableHead() && (React__namespace.createElement(Head, { className: "rdt_TableHead", role: "rowgroup", fixedHeader: fixedHeader },
                            React__namespace.createElement(HeadRow, { className: "rdt_TableHeadRow", role: "row", dense: dense },
                                keepExpandableFirst ? (React__namespace.createElement(React__namespace.Fragment, null,
                                    expandableHeaderCol,
                                    selectableHeaderCol)) : (React__namespace.createElement(React__namespace.Fragment, null,
                                    selectableHeaderCol,
                                    expandableHeaderCol)),
                                tableColumnsBasedOnTable.map(column => (React__namespace.createElement(Column, { key: column.id, column: column, selectedColumn: selectedColumn, disabled: progressPending || sortedData.length === 0, pagination: pagination, paginationServer: paginationServer, persistSelectedOnSort: persistSelectedOnSort, selectableRowsVisibleOnly: selectableRowsVisibleOnly, sortDirection: sortDirection, sortIcon: sortIcon, sortServer: sortServer, onSort: handleSort, onDragStart: handleDragStart, onDragOver: handleDragOver, onDragEnd: handleDragEnd, onDragEnter: handleDragEnter, onDragLeave: handleDragLeave, draggingColumnId: draggingColumnId, currentSortColumnId: currentSortColumnId, currentSortDirection: currentSortDirection })))))),
                        ...
                        !progressPending && sortedData.length > 0 && (React__namespace.createElement(Body, { className: "rdt_TableBody", role: "rowgroup" }, tableRows.map((row, i) => {
                            ...
                            return (React__namespace.createElement(Row, { id: id, key: id, keyField: keyField, "data-row-id": id, columns: tableColumnsBasedOnTable, row: row, rowCount: sortedData.length, rowIndex: i, selectableRows: selectableRows, expandableRows: expandableRows, expandableIcon: expandableIcon, highlightOnHover: highlightOnHover, pointerOnHover: pointerOnHover, dense: dense, expandOnRowClicked: expandOnRowClicked, expandOnRowDoubleClicked: expandOnRowDoubleClicked, expandableRowsComponent: expandableRowsComponent, expandableRowsComponentProps: expandableRowsComponentProps, expandableRowsHideExpander: expandableRowsHideExpander, defaultExpanderDisabled: expanderDisabled, defaultExpanded: expanderExpander, expandableInheritConditionalStyles: expandableInheritConditionalStyles, conditionalRowStyles: conditionalRowStyles, selected: selected, expanded: expanded, selectableRowsHighlight: selectableRowsHighlight, selectableRowsComponent: selectableRowsComponent, selectableRowsComponentProps: selectableRowsComponentProps, selectableRowDisabled: selectableRowDisabled, expandableRowDisabled: expandableRowDisabled, selectableRowsSingle: selectableRowsSingle, expandableRowsSingle: expandableRowsSingle, striped: striped, keepExpandableFirst: keepExpandableFirst, onRowExpandToggled: onRowExpandToggled, onRowClicked: handleRowClicked, onRowDoubleClicked: handleRowDoubleClicked, onSelectedRow: handleSelectedRow, onExpandedRow: handleExpandedRow, draggingColumnId: draggingColumnId, onDragStart: handleDragStart, onDragOver: handleDragOver, onDragEnd: handleDragEnd, onDragEnter: handleDragEnter, onDragLeave: handleDragLeave }));
                        })))),
                    footer && React__namespace.createElement("div", null, footer))),
            freezedColumns.length > 0 && (React__namespace.createElement("div", { style: {
                    position: 'absolute',
                    top: 0,
                    left: 0,
                    maxWidth: '100%',
                    ...
                    showTableHead() && (React__namespace.createElement(Head, { className: "rdt_TableHead", role: "rowgroup", fixedHeader: fixedHeader },
                        React__namespace.createElement(HeadRow, { className: "rdt_TableHeadRow", role: "row", dense: dense },
                            keepExpandableFirst ? (React__namespace.createElement(React__namespace.Fragment, null,
                                ...
                                selectableHeaderCol,
                                expandableHeaderCol)),
                            freezedColumns.map(column => (React__namespace.createElement(Column, { key: column.id, column: column, selectedColumn: selectedColumn, disabled: progressPending || sortedData.length === 0, pagination: pagination, paginationServer: paginationServer, persistSelectedOnSort: persistSelectedOnSort, selectableRowsVisibleOnly: selectableRowsVisibleOnly, sortDirection: sortDirection, sortIcon: sortIcon, sortServer: sortServer, onSort: handleSort, onDragStart: handleDragStart, onDragOver: handleDragOver, onDragEnd: handleDragEnd, onDragEnter: handleDragEnter, onDragLeave: handleDragLeave, draggingColumnId: draggingColumnId, currentSortColumnId: currentSortColumnId, currentSortDirection: currentSortDirection })))))),
                    !progressPending && sortedData.length > 0 && (React__namespace.createElement(Body, { className: "rdt_TableBody", role: "rowgroup" }, tableRows.map((row, i) => {
                        const key = prop(row, keyField);
                        const id = isEmpty(key) ? i : key;
                        const selected = isRowSelected(row, selectedRows, keyField);
                        ...
                        const expanderExpander = !!(expandableRows && expandableRowExpanded && expandableRowExpanded(row));
                        const expanderDisabled = !!(expandableRows && expandableRowDisabled && expandableRowDisabled(row));
                        return (React__namespace.createElement(Row, { id: id, key: id, keyField: keyField, "data-row-id": id, columns: freezedColumns, row: row, rowCount: sortedData.length, rowIndex: i, selectableRows: selectableRows, expandableRows: expandableRows, expandableIcon: expandableIcon, highlightOnHover: highlightOnHover, pointerOnHover: pointerOnHover, dense: dense, expandOnRowClicked: expandOnRowClicked, expandOnRowDoubleClicked: expandOnRowDoubleClicked, expandableRowsComponent: expandableRowsComponent, expandableRowsComponentProps: expandableRowsComponentProps, expandableRowsHideExpander: expandableRowsHideExpander, defaultExpanderDisabled: expanderDisabled, defaultExpanded: expanderExpander, expandableInheritConditionalStyles: expandableInheritConditionalStyles, conditionalRowStyles: conditionalRowStyles, selected: selected, expanded: expanded, selectableRowsHighlight: selectableRowsHighlight, selectableRowsComponent: selectableRowsComponent, selectableRowsComponentProps: selectableRowsComponentProps, selectableRowDisabled: selectableRowDisabled, expandableRowDisabled: expandableRowDisabled, selectableRowsSingle: selectableRowsSingle, expandableRowsSingle: expandableRowsSingle, striped: striped, keepExpandableFirst: keepExpandableFirst, onRowExpandToggled: onRowExpandToggled, onRowClicked: handleRowClicked, onRowDoubleClicked: handleRowDoubleClicked, onSelectedRow: handleSelectedRow, onExpandedRow: handleExpandedRow, draggingColumnId: draggingColumnId, onDragStart: handleDragStart, onDragOver: handleDragOver, onDragEnd: handleDragEnd, onDragEnter: handleDragEnter, onDragLeave: handleDragLeave }));
                    })))),
                footer && React__namespace.createElement("div", null, footer)))),
